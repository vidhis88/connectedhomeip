...
/*
 * ChipStack - a singleton object shared between all instances of
...
public final class ChipStack {
	...
	private static final String TAG = ChipStack.class.getSimpleName();
	public static final int INITIAL_CONNECTIONS = 4;

	private static class BleMtuDenylist {
		/**
		 * Will be set at initialization to indicate whether the device on which
		 * this code is being run is known to indicate unreliable MTU values for
		 * Bluetooth LE connections.
		 */
		static final boolean BLE_MTU_DENYLISTED;

		/**
		 * If {@link #BLE_MTU_DENYLISTED} is true, then this is the fallback MTU to
		 * use for this device
		 */
	...
	private static final ChipStack sInstance = new ChipStack();
		...
		mGattCallback = new BluetoothGattCallback() {
          ...
          BluetoothGattCharacteristic characteristic, int status) {
				...
				if (status != BluetoothGatt.GATT_SUCCESS) {
					Log.e(TAG, "onCharacteristicWrite for " + characteristic.getUuid().toString() +
              " failed with status: " + status);
				...
				if (connId > 0) {
					handleWriteConfirmation(connId, svcIdBytes, charIdBytes,
          ...
          BluetoothGattCharacteristic characteristic) {
			...
			public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor desc, int status) {
				...
				if (status != BluetoothGatt.GATT_SUCCESS) {
					Log.e(TAG, "onDescriptorWrite for " + desc.getUuid().toString() + " failed with status: "
              + status);
				}
				...
				if (desc.getValue() == BluetoothGattDescriptor.ENABLE_INDICATION_VALUE) {
					handleSubscribeComplete(connId, svcIdBytes, charIdBytes,
              status == BluetoothGatt.GATT_SUCCESS);
				} else if (desc.getValue() == BluetoothGattDescriptor.DISABLE_NOTIFICATION_VALUE) {
					handleUnsubscribeComplete(connId, svcIdBytes, charIdBytes,
              status == BluetoothGatt.GATT_SUCCESS);
				} else {
	...
	public static ChipStack getInstance() {
		return sInstance;
	}
	...
	public static void onNotifyChipConnectionClosed(int connId) {
		ChipDeviceController deviceController = ChipStack.getInstance().getConnection(connId);
		deviceController.onNotifyChipConnectionClosed(connId);
	}

	public static boolean onSendCharacteristic(int connId, byte[] svcId, byte[] charId,
	...
	static {
		System.loadLibrary("ChipDeviceController");
	}

	private native void handleWriteConfirmation(int connId, byte[] svcId, byte[] charId,
      boolean success);
	private native void handleIndicationReceived(int connId, byte[] svcId, byte[] charId,
      byte[] data);
	private native void handleSubscribeComplete(int connId, byte[] svcId, byte[] charId,
      boolean success);
	private native void handleUnsubscribeComplete(int connId, byte[] svcId, byte[] charId,
      boolean success);
